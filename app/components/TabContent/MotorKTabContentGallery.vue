<template>
    <section id="gallery" class="tabs__content">
        ...
        <!-- <script type="text/template" id="tpl--gallery-box">
          ...
          <div class="gallery__el layout__item js-open-detail">
            <figure class="project">
              <img src="<% this.data[i].projectImage.mid %>" alt="" class="project__media">
              <figcaption class="project__desc">
                <header class="project__head layout">
                        <span class="project__user-name layout__item">
                          <% this.data[i].userName %>
                        </span>
                  <span class="project__title layout__item">
                          <% this.data[i].projectTitle %>
                        </span>
                </header>
                <p class="project__text">
                  <% MTRK.sanitizeString(this.data[i].projectDesc) %>
                </p>
              </figcaption>
              <footer class="project__actions">
                <div class="do-like">
                        <span class="project__likes">
                          <span class="project__likes__amount"><% this.data[i].projectLike %></span> Like
                        </span>
                  <button data-likes="<% this.data[i].projectLike %>"
                      data-id="<% this.data[i].boxId %>"
                      class="project__icon js-vote-project <% this.data[i].hasVoted ? "
                      is-selected
                  " : " " %>">&nbsp;</button>
                </div>
                <div class="do-share">
                        <span class="do-share__wrapper">
                          <a class="btn--rounded btn--fb"
                          href="<% MTRK.createShareURL.fb(this.data[i].boxId) %>"
                          target="_blank">
                            <svg class="icon icon--light" viewBox="0 0 1024 1024"
                          ...
                          <a class="btn--rounded btn--tw"
                          href="<% MTRK.createShareURL.tw(this.data[i].boxId) %>"
                          target="_blank">
                            <svg class="icon icon--light" viewBox="0 0 1024 1024"
                              version="1.1" xmlns="http://www.w3.org/2000/svg">
                              <title>twitter</title>
                          ...
                          <a class="btn--rounded btn--pint"
                          href="<% MTRK.createShareURL.pin(this.data[i].projectDesc, this.data[i].projectImage.mid) %>"
                          target="_blank">
                            <svg class="icon icon--light" viewBox="0 0 1024 1024"
                              version="1.1" xmlns="http://www.w3.org/2000/svg">
